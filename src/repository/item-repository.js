...
class ItemRepository {
  ...
  async createItem(item_type_id, storage_space_id, expiration_date) {
    try {
      if (
        ...
        (await this.storageSpaceRepository.storageSpaceExists(
          storage_space_id
        )) &&
        !(await this.storageSpaceRepository.isStorageSpaceFull(
          storage_space_id
        )) &&
        new Date(expiration_date) > new Date() &&
        ((await this.storageSpaceRepository.isStorageSpaceRefrigerated(
          storage_space_id
        )) ||
          !(await this.itemTypeRepository.isItemTypeRefrigerated(item_type_id)))
      ) {
        ...
        await this.storageSpaceRepository.increaseStorageSpaceCount(
          storage_space_id
        );
  ...
  async relocateItem(item_id, destination_storage_space_id) {
    try {
      ...
      const destinationSpace = await StorageSpace.findById(
        destination_storage_space_id
      );
      ...
      if (
        item.item_type_id.requires_refrigeration &&
        !destinationSpace.refrigeration
      ) {
